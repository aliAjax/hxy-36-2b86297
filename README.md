...
- **Node.js >= 20.19.0**（jsdom 29.x 要求，推荐 22 LTS 或 24+）
- npm >= 7

> 主要依赖版本约束：
> - Vite 6.x：支持 Node.js 18 / 20 / 22+
> - Vitest 4.x：要求 Node.js >= 20.0.0、Vite >= 6.0.0
> - jsdom 29.x：要求 Node.js >= 20.19.0 / 22.13.0 / 24+
...
脚本在 Node 环境中运行时没有浏览器的 `localStorage`，因此可能输出 `zustand persist middleware` 存储不可用警告。该警告只表示测试数据无法持久化到浏览器存储，不影响脚本中的内存状态验证结果；脚本结束前会自动清理验证数据。
